...
function Completes() {
    ...
    const {id} = router;
    const completeData = dataUser?.todo.filter((element)=> element.complete === true)
    useEffect(() => {
    ...
    <Image src={img} height={140} width={140} className='imageFormTask'/> 
      <div className="filterCategories">
          <p className={istokFont.className} style={{
              ...
              <button className={`${montserrat.className} buttonForm`} style={{fontWeight:600}} onClick={()=>navigate.push(`/perfil/${id}/incomplete`) }>
                <CancelScheduleSendIcon  style={{fontSize:25 , color:"white", marginRight:20}}/>
                Incompletes
              </button>
              </div>
